...
import os
import uuid
import utils.user_management_util as user_management_util
...
class CognitoIdentityProviderManagement():
    __DEFAULT_CALLBACK_URL='http://localhost'
    ...
    def create_control_plane_idp(self, event):
        ...
        if control_plane_callback_url is None or len(control_plane_callback_url) == 0:
            control_plane_callback_url = self.__DEFAULT_CALLBACK_URL
    ...
    def __create_user_pool(self, user_pool_name, control_plane_site_url):
        ...
        if (control_plane_site_url == self.__DEFAULT_CALLBACK_URL):
            email_message = ''.join(["Interact with control plane using",
